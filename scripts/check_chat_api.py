...
Sends one throwaway chat completion through the same client construction path the
application uses, so proxy and base-URL problems surface here instead of mid-run.
This is not a pytest module; run it directly with `python scripts/check_chat_api.py`.
"""
