...
def first_text(names: tuple[str, ...], default: str) -> str:
...
def first_optional(names: tuple[str, ...]) -> str | None:
...
def token(name: str, default: str) -> str:
    ...
    return text(name, default).strip().lower()
...
def path(name: str, default: str) -> pathlib.Path:
    """Return a filesystem-path setting as a :class:`~pathlib.Path`."""
    return pathlib.Path(text(name, default))
