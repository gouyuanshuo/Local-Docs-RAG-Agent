...
Discovery is separated from ingestion because it is also the answer to "what can
this agent see?" — `tools.list_documents` and the `/api/documents` endpoint use
it without touching the index at all. Keeping it here means the file set an
answer is grounded in and the file set reported to the user can never disagree.
"""
...
def require_docs_directory(docs_dir: pathlib.Path) -> None:
    ...
    if not docs_dir.exists():
        raise exceptions.ConfigurationError(
            ...
            action_hint=(
                "Create the directory or set DOCS_DIR to an existing directory."
            ),
        )
    if not docs_dir.is_dir():
        raise exceptions.ConfigurationError(
            f"DOCS_DIR is not a directory: {docs_dir}"
        )


def _is_excluded(
    path: pathlib.Path, docs_dir: pathlib.Path, exclude_patterns: list[str]
) -> bool:
    # Patterns are matched against both forms so `notes/*` and `docs/notes/*`
    # both work.
