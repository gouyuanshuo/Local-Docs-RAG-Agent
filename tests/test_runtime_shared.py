...
def test_context_labels_start_at_column_zero() -> None:
    context = runtime_shared.build_answer_context([_hit()])

    labelled_lines = [
        line
        for line in context.splitlines()
        if line.startswith(
            ("[S", "source:", "title:", "chunk_index:", "content:")
        )
    ]
...
def test_extractive_fallback_recovers_context_from_a_multiline_chunk() -> None:
    # A chunk body almost always contains a line at column 0. If the context
    # template leaves its labels indented, the fallback parser finds nothing and
    # wrongly claims there was no supporting context even though retrieval
    # succeeded.
    provider = chat.OpenAICompatibleChatProvider(api_key=None, model="unused")
...
def test_fallback_still_reports_when_context_is_genuinely_empty() -> None:
    ...
    answer = provider.answer(
        question="anything", context=runtime_shared.build_answer_context([])
    )
...
def test_tool_results_include_the_score_and_full_text() -> None:
    rendered = runtime_shared.format_tool_search_results([_hit(score=0.4242)])
...
def test_merge_hits_keeps_first_occurrence_only() -> None:
    ...
    runtime_shared.merge_hits(
        existing,
        [_hit(chunk_id="a"), _hit(chunk_id="b", source_path="other.md")],
    )

    assert [hit.chunk.chunk_id for hit in existing] == ["a", "b"]
    assert runtime_shared.collect_citations(existing) == ["doc.md", "other.md"]
